...
public class EntryList extends JPanel {
    ...
    protected boolean mCreateViewName = false;
    protected boolean mLPCreate = false;
    ...
    protected JCheckBox mLPModle;
    protected JCheckBox mLPModleViewName;
    protected JCheckBox msplitOnclickMethodsCheck;
    protected JLabel mHolderLabel;
    protected JLabel mLpLabel;
    protected JLabel mLpLabelViewName;
    ...
    protected void addButtons() {
		...
		*/
        ...
        mLPModle = new JCheckBox();
        mLPModle.setPreferredSize(new Dimension(32, 26));
        mLPModle.setSelected(mLPCreate);
        mLPModle.addChangeListener(new CheckLPCreateListener());

        mLpLabel = new JLabel();
        mLpLabel.setText("Nativity");

        JPanel lpPanel = new JPanel();
        lpPanel.setLayout(new BoxLayout(lpPanel, BoxLayout.LINE_AXIS));
        lpPanel.setBorder(BorderFactory.createEmptyBorder(0, 10, 0, 10));
        lpPanel.add(mLPModle);
        lpPanel.add(mLpLabel);
        lpPanel.add(Box.createHorizontalGlue());
        add(lpPanel, BorderLayout.PAGE_END);

        mLPModleViewName = new JCheckBox();
        mLPModleViewName.setPreferredSize(new Dimension(32, 26));
        mLPModleViewName.setSelected(mCreateViewName);
        mLPModleViewName.addChangeListener(new CheckFindListener());

        mLpLabelViewName = new JLabel();
        mLpLabelViewName.setText("findViewByid");

        JPanel lpPanelFind = new JPanel();
        lpPanelFind.setLayout(new BoxLayout(lpPanelFind, BoxLayout.LINE_AXIS));
        lpPanelFind.setBorder(BorderFactory.createEmptyBorder(0, 10, 0, 10));
        lpPanelFind.add(mLPModleViewName);
        lpPanelFind.add(mLpLabelViewName);
        lpPanelFind.add(Box.createHorizontalGlue());
        add(lpPanelFind, BorderLayout.PAGE_END);
    ...
    public class CheckLPCreateListener implements ChangeListener {

        @Override
        public void stateChanged(ChangeEvent event) {
            mLPCreate = mLPModle.isSelected();
        }
    } public class CheckFindListener implements ChangeListener {

        @Override
        public void stateChanged(ChangeEvent event) {
            mCreateViewName = mLPModleViewName.isSelected();
        }
    }
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            if (valid) {
                if (mConfirmListener != null) {
                    mConfirmListener.onConfirm(mProject, mEditor, mElements, mPrefix, mCreateHolder, mLPCreate,mCreateViewName, msplitOnclickMethodsCheck.isSelected());
                }
            }
        }
    }
